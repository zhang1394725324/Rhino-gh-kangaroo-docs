...
function renderRichDetail(item, details) {
    ...
    let descriptionHtml = '';
    if (lang === 'cn') {
        descriptionHtml = details.description_cn || item.desc_cn || '暂无描述';
    } else {
        descriptionHtml = details.description_en || item.desc_en || 'No description';
    }
    ...
    // 注意：这里的 ${descriptionHtml} 没有使用 escapeHtml，以支持富文本
